...
// Cape geometry for Capitão Verde
const CAPE_GEO = new THREE.PlaneGeometry(0.7, 1.0);
// V-Chevron symbols (white) for chest and helmet
const CHEVRON_GEO = new THREE.BoxGeometry(0.35, 0.06, 0.02); // Horizontal bar for V shape
const BELT_GEO = new THREE.BoxGeometry(0.35, 0.08, 0.18); // Yellow belt around waist
...
export const Player: React.FC = () => {
    ...
    // Memoized Materials - Capitão Verde color scheme (based on reference image)
    const { armorMaterial, darkArmorMaterial, whiteMaterial, beltMaterial, jointMaterial, glowMaterial, shadowMaterial, capeMaterial } = useMemo(() => {
        // Forest green for main armor, golden for immortality power-up
        const mainArmorColor = isImmortalityActive ? '#ffd700' : '#1b5e20';  // Dark forest green
        const darkArmorColor = isImmortalityActive ? '#b8860b' : '#0d3818';  // Even darker green for contrast


        return {
            armorMaterial: new THREE.MeshStandardMaterial({ color: mainArmorColor, roughness: 0.3, metalness: 0.6 }),
            darkArmorMaterial: new THREE.MeshStandardMaterial({ color: darkArmorColor, roughness: 0.2, metalness: 0.7 }),
            whiteMaterial: new THREE.MeshStandardMaterial({ color: '#ffffff', roughness: 0.4, metalness: 0.2 }), // For V chevrons
            beltMaterial: new THREE.MeshStandardMaterial({ color: '#ffd600', roughness: 0.3, metalness: 0.9 }), // Yellow/gold belt
            jointMaterial: new THREE.MeshStandardMaterial({ color: '#111111', roughness: 0.7, metalness: 0.5 }),
            glowMaterial: new THREE.MeshBasicMaterial({ color: isImmortalityActive ? '#ffffff' : '#ffea00' }),
            shadowMaterial: new THREE.MeshBasicMaterial({ color: '#000000', opacity: 0.3, transparent: true }),
            capeMaterial: new THREE.MeshStandardMaterial({
                color: '#d84315',  // Orange-red cape like in reference
    ...
    return (
        <group ref={groupRef} position={[0, 0, 0]}>
            {/* Rotate entire body 180° so character faces away (runs into distance) */}
            <group ref={bodyRef} position={[0, 1.1, 0]} rotation={[0, Math.PI, 0]}>

                {/* Torso */}
                <mesh castShadow position={[0, 0.2, 0]} geometry={TORSO_GEO} material={armorMaterial} />

                {/* White V-Chevron on chest - Capitão Verde's signature symbol */}
                <group position={[0, 0.35, 0.14]}>
                    {/* Top V chevron */}
                    <mesh position={[-0.08, 0.05, 0]} rotation={[0, 0, -0.5]} geometry={CHEVRON_GEO} material={whiteMaterial} />
                    <mesh position={[0.08, 0.05, 0]} rotation={[0, 0, 0.5]} geometry={CHEVRON_GEO} material={whiteMaterial} />
                    {/* Middle V chevron */}
                    <mesh position={[-0.1, -0.05, 0]} rotation={[0, 0, -0.5]} geometry={CHEVRON_GEO} material={whiteMaterial} />
                    <mesh position={[0.1, -0.05, 0]} rotation={[0, 0, 0.5]} geometry={CHEVRON_GEO} material={whiteMaterial} />
                </group>

                {/* Yellow belt around waist */}
                <mesh position={[0, -0.05, 0]} geometry={BELT_GEO} material={beltMaterial} />

                {/* Red Cape - Capitão Verde's signature flowing cape */}
                <mesh
                    position={[0, 0.1, -0.35]}
                    rotation={[0.2, 0, 0]}
                    geometry={CAPE_GEO}
                    material={capeMaterial}
                    castShadow
                />
                ...
                <group ref={headRef} position={[0, 0.6, 0]}>
                    ...
                    {/* White V symbol on helmet forehead */}
                    <group position={[0, 0.1, 0.16]}>
                        <mesh position={[-0.04, 0, 0]} rotation={[0, 0, -0.6]} geometry={CHEVRON_GEO} scale={[0.5, 0.8, 1]} material={whiteMaterial} />
                        <mesh position={[0.04, 0, 0]} rotation={[0, 0, 0.6]} geometry={CHEVRON_GEO} scale={[0.5, 0.8, 1]} material={whiteMaterial} />
                    </group>
                </group>
